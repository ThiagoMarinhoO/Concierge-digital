...
jQuery(document).ready(function($) {
    ...
    var formElement = $('#concierge-form');
    var chatbotButton = $('#concierge-test-chatbot');
    var resultContainer = $('#concierge-test-result');
    ...
    chatbotButton.on('click', function(e) {
        ...
        if (jsonlData.length === 0) {
            resultContainer.html('<p style="color: red;">Por favor, preencha os campos do formulário.</p>');
            return;
        }
        ...
        // Exibir mensagem de carregamento
        resultContainer.html('<p>Processando... Por favor, aguarde.</p>');

        $.ajax({
            url: conciergeAjax.ajax_url, // URL absoluta
            method: 'POST',
            processData: true,
            data: {
                action: 'concierge_process_form', // Nome da ação configurada no backend
                jsonl_data: jsonlData.map(item => (item)).join('\n'),
                nonce: conciergeAjax.nonce, // Segurança do WordPress
            },
            success: function(response) {
                console.log('Resposta do Backend:', response);
                resultContainer.html('<p style="color: green;">Chatbot gerado com sucesso!</p>');
            },
            error: function(xhr, status, error) {
                resultContainer.html('<p style="color: red;">Erro ao se conectar ao servidor. Por favor, tente novamente.</p>');
                console.error('Erro na Requisição:', error);
            }
        });
