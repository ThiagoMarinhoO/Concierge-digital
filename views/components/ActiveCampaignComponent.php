...
class ActiveCampaignComponent
{
    ...
    public static function render()
    {
        ...
        $currentAssistant = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}chatbot WHERE user_id = %s",
                $user_id
            )
        );

        $assistantId = $currentAssistant ? $currentAssistant->id : null;

        $campaignVariables = $wpdb->get_row(
            $wpdb->prepare(
...
?>
        ...
        <script>
            window.addEventListener('DOMContentLoaded', function() {
                document.querySelector('#active_campaign_form').addEventListener('submit', async function(event) {
                    event.preventDefault();
    
                    const apiUrl = document.querySelector('#active_campaign_api_url').value;
                    const apiKey = document.querySelector('#active_campaign_api_key').value;
                    ...
                    try {
                        const response = await fetch('<?php echo admin_url('admin-ajax.php'); ?>', {
                            ...
                            body: new URLSearchParams({
                                action: 'save_active_campaign_credentials',
                                api_url: apiUrl,
                                api_key: apiKey,
                                assistant_id: '<?php echo esc_js($assistantId); ?>'
                            })
                        });
                        ...
                        swal.fire({
                            title: 'Erro ao salvar credenciais',
                            icon: 'error',
                            text: 'Erro ao criar credencial. Tente novamente mais tarde. Ou entre em contato com o suporte.',
                            confirmButtonText: 'Fechar'
                        });
                        return;
                    }
        ...
        <div class="active-campaign-component-wrapper p-4 !bg-white !shadow-sm rounded-lg !mt-8">
            <div class="container flex justify-center p-3">
                <svg class="h-8" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 516 53" focusable="false" role="img" aria-label="ActiveCampaign" aria-hidden="true">
...
// Quando o WordPress encontrar [active_campaign], ele chamará o método 'render' da classe ActiveCampaignComponent.
add_shortcode('active_campaign_component', [ActiveCampaignComponent::class, 'render']);
