...
<script>
	window.addEventListener('DOMContentLoaded', (event) => {
		...
		if (userIsOperator) {
			swal.fire({
				title: 'Atenção!',
				text: 'Você não pode acessar essa área como operador.',
				icon: 'warning',
...
<div class="assistantContainer" data-assistantID="<?= $assistant['id'] ?>">
	<div id="tabs-container" class="grid grid-cols-1 md:grid-cols-3 gap-4 relative">
		...
		<?php foreach ($categories as $index => $category): ?>
			...
			$firstUnlocked = false;
			?>
			<button data-current="<?php echo $i === 1 ? 'true' : 'false' ?>" data-tab="<?= esc_attr($tabName) ?>"
				data-locked="<?= $isLocked ? 'true' : 'false' ?>"
				class="tab-btn rounded-md <?= $isLocked ? 'cursor-not-allowed opacity-50' : 'cursor-pointer' ?> p-6 shadow-md bg-white text-gray-700 font-bold border-b-2 border-transparent <?= $isLocked ? '' : 'hover:border-gray-800' ?> focus:outline-none"
				data-tab-num="<?php echo $i ?>">
	...
	<div id="tabs-content-container" class="min-h-[560px]">
				...
				<?php if ($category['has_tabs']): ?>
					<div x-data="{ selectedTab: 'fast' }" class="w-full">
						<div x-on:keydown.right.prevent="$focus.wrap().next()"
							x-on:keydown.left.prevent="$focus.wrap().previous()" class="flex gap-2" role="tablist"
							aria-label="tab options">
							<button x-on:click="selectedTab = 'fast'" x-bind:aria-selected="selectedTab === 'fast'"
								x-bind:tabindex="selectedTab === 'fast' ? '0' : '-1'"
								x-bind:class="selectedTab === 'fast' ? 'font-bold text-black border-b-2 border-black dark:border-white dark:text-white' : 'text-neutral-600 font-medium dark:text-neutral-300 dark:hover:border-b-neutral-300 dark:hover:text-white hover:border-b-2 hover:border-b-neutral-800 hover:text-neutral-900'"
								class="h-min px-4 py-2 text-sm" type="button" role="tab"
								aria-controls="tabpanelFast">Rápida</button>
							<button x-on:click="selectedTab = 'custom'" x-bind:aria-selected="selectedTab === 'custom'"
								x-bind:tabindex="selectedTab === 'custom' ? '0' : '-1'"
								x-bind:class="selectedTab === 'custom' ? 'font-bold text-black border-b-2 border-black dark:border-white dark:text-white' : 'text-neutral-600 font-medium dark:text-neutral-300 dark:hover:border-b-neutral-300 dark:hover:text-white hover:border-b-2 hover:border-b-neutral-800 hover:text-neutral-900'"
								class="h-min px-4 py-2 text-sm" type="button" role="tab"
								aria-controls="tabpanelCustom">Personalizada</button>
						</div>
						<div class="px-2 py-4 text-neutral-600 dark:text-neutral-300">
							...
							<div x-cloak x-show="selectedTab === 'custom'" id="tabpanelcustom" role="tabpanel"
								aria-label="custom">
				...
				<?php else: ?>
					<div class="flex items-center justify-center gap-12">
						<div>
							<?php foreach ($questionsByCategory[$category['title']] as $index => $question): ?>
								<?php if (!empty($questionsByCategory)): ?>
									<div class="question-block relative !grid !grid-cols-3">
										<label for="question-<?php echo esc_attr($index); ?>"
										...
										<?php elseif ($field_type === 'file'): ?>
											...
											<input type="file" multiple
												accept="<?php echo ($question['title'] !== 'Adicione Áudios:') ? '.txt,.docx,.pdf' : 'audio/*'; ?>"
												id="question-<?php echo esc_attr($index); ?>"
												name="question_<?php echo esc_attr($question['id']); ?>"
												class="py-2 px-2.5 border border-gray-100 rounded-lg w-full my-2 <?php echo $name . ' ' . $wellcome_message ?>"
												<?php echo $required ?>>
											<button
												class="add-more-files absolute right-0 top-0 !w-fit !bg-gray-300 !border-none !hover:bg-gray-400 !p-2 !rounded-full">
											...
											<div
												class="space-y-2 gap-2 file-name-container col-span-3 border border-gray-300 rounded-lg divide-y">
											</div>
											<?php if ($question['title'] === 'Documentos anexos'): ?>
												<div class="attachment-instructions-container col-span-3">
							...
							<div class="">
								<div class="video-container mb-4">
									<video controls class="w-full rounded-lg size-64">
										<source src="<?php echo $category['video_url'] ?>" type="video/mp4">
									</video>
								</div>
							</div>
						<?php endif ?>
			...
			if ($has_active_chatbot): ?>
				<div class="relative flex flex-col items-center justify-center min-h-screen text-gray-800 p-10">
					...
					<div class="flex flex-col flex-grow w-full max-w-xl bg-white shadow-xl rounded-lg overflow-hidden">
						...
						<?php
						$assistantObj = json_decode($assistant['assistant'], true);
						?>

						<!-- Container do chat -->
						<div class="flex flex-col flex-grow h-0 p-4 overflow-auto chatContainer"
							data-assistant-id="<?php echo esc_attr($assistant['id']); ?>" data-session-id="">
							<?php if (!empty($assistantObj['metadata']['welcome_message'])): ?>
								<div class="flex w-full mt-2 space-x-3 max-w-xs">
									<div class="flex-shrink-0 h-10 w-10 rounded-full bg-gray-300">
										<img src="<?php echo $assistantObj['metadata']['assistant_image']; ?>" alt=""
											class="w-10 !h-10 object-cover !rounded-full">
									</div>
									<div>
										<div class="bg-gray-300 p-3 rounded-r-lg rounded-bl-lg text-sm">
											<?php echo $assistantObj['metadata']['welcome_message']; ?>
										</div>
									</div>
								</div>
							<?php endif; ?>
		...
		<div id="Download-content" class="tab-content hidden absolute inset-0 bg-white p-4">
				...
				<div class="w-1/2">
					<div id="clipboardSection"
						...
						<button id="copyButton"
							...
							<svg id="copyIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
								fill="currentColor" class="size-4" aria-hidden="true">
							...
							<svg id="successIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"
								fill="currentColor" class="size-4 fill-green-500 hidden">
								<path fill-rule="evenodd"
			...
			<?php if (empty($whatsappInstance)): ?>
				<button type="button" name="" id="conectar-whatsapp"
					class="mb-4 rounded hover:!bg-green-700 hover:!text-white">Conectar ao whatsapp</button>
			<?php endif; ?>

			<?php if (($whatsappInstance)): ?>
				<div id="whatsapp-instance" class="flex flex-col rounded-lg border bg-card text-card-foreground shadow-sm"
					data-instanceName="<?php echo esc_attr($whatsappInstance->getInstanceName()); ?>">
					<div class="flex flex-col space-y-1.5 p-6">
						<div class="flex w-full flex-row items-center justify-between gap-4" href="">
							<h3 class="text-wrap font-semibold">
								<?php echo esc_html($whatsappInstance->getInstanceName()); ?></h3>
						</div>
					</div>
					<div class="flex items-center p-6 pt-0 justify-between">
						<div
							class="!hidden inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-primary !text-primary-foreground hover:bg-primary/80">
							Connecting</div><button id="delete_instance"
							class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 !bg-red-500 !text-white hover:!bg-red-500/90 h-9 rounded-md px-3"><span>Delete</span></button>
					</div>
				</div>
			<?php endif; ?>
		</div>
